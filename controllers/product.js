...
exports.create = (req, res) => {
  ...
  form.parse(req, (err, formData, files) => {
    ...
    // check for all fields(formData)
    const { name, description, price, category, quantity, shipping } = formData
...
exports.update = (req, res) => {
  ...
  form.parse(req, (err, formData, files) => {
    ...
    let product = req.product
    product = _.extend(product, formData)
    // 1kb = 1000
    // 1mb - 1000000
    if(files.photo) {
      // console.log('FILES PHOTO: ', files.photo)
...
// if no params are sent, then all prodcuts are returned
